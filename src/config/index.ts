...
export function logConfigSummary() {
  ...
  console.log(
    `  Dry Run: ${config.dryRun ? "✅ ATIVO" : "❌ DESATIVO"}`
  );
  console.log(
    `  Validações: ${config.validateAllActions ? "✅ ATIVAS" : "❌ INATIVAS"}`
  );
  console.log(
    `  Audit Log: ${config.monitoring.enableAuditLog ? "✅ ATIVO" : "❌ INATIVO"}`
  );
  console.log(
    `  Rate Limit: ${config.rateLimiting.maxRequestsPerSecond} req/s`
  );
  console.log(
    `  Project Types: ${config.cascade.projectTypes.join(", ")}`
  );
  ...
  console.log(
    `  Isolation Mode: ${config.isolation.isolationMode.toUpperCase()}`
  );
  console.log(
    `  Webhook Conflict Check: ${config.webhooks.collisionCheck ? "✅" : "❌"}`
  );
  console.log(
    `  Known Webhooks: [${config.webhooks.knownWebhookIds.join(", ")}]`
  );
  console.log();
}
